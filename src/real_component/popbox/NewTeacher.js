...
import NewTeacherTemButton from "./NewTeacherTermButton";
...
const NewTeacher = (props) => {
  ...
  function ToggleTerm(event, termId) {
    const searchRes = selectedTerms.find((termObject) => {
      return termObject.termId == termId;
    });
    if (searchRes == undefined) {
      setSelectedTerms([...selectedTerms, { termId: termId }]);
      return true;
    } else {
      const newTerms = [
        ...selectedTerms.filter((term) => {
          return term.termId == termId ? false : true;
        }),
      ];
  ...
  function SendInformation() {
    props.FillInformation({
      ...
      mainTerm: mainTerm,
      booksId: [...selectedTerms.map((termInfo)=>termInfo.termId)],
    });
  }
  return (
    <div
    ...
    >
      <div className="middle-div w-2/3 h-2/3 border-2 border-t border-gray-700 flex flex-col justify-between ">
        ...
        <button
          ...
          type="button"
        >
          <i className="fa-sharp fa-solid fa-plus"></i> Add NewTeacher
        </button>
      </div>
    </div>
  );
